...
class SAASGP(object):
    ...
    def __init__(
        ...
        if observation_variance < 0.0:
            raise ValueError(
                "The hyperparameter observation_variance should be non-negative."
            )
    ...
    def model(self, X, Y):
        ...
        noise = (
            ...
            if self.learn_noise
            else self.observation_variance
        )
        ...
        inv_length_sq = numpyro.sample(
            "_kernel_inv_length_sq", dist.HalfCauchy(jnp.ones(P))
        )
    ...
    def compute_choleskys(self, chunk_size=8):
        def _cholesky(var, inv_length_sq, noise):
            k_XX = self.kernel(
                self.X_train, self.X_train, var, inv_length_sq, noise, True
            )
