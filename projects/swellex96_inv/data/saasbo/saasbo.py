...
def optimize_ei(gp, y_target, xi=0.0, num_restarts_ei=3, num_init=8000):
    ...
    X_rand[0, :] = np.clip(
        x_best + 0.001 * np.random.randn(1, dim), a_min=0.0, a_max=1.0
    )
...
def run_saasbo(
    ...
    if lb.shape != ub.shape or lb.ndim != 1:
        raise ValueError(
            "The lower/upper bounds lb and ub must have the same shape and be D-dimensional vectors."
        )
    if alpha <= 0.0:
    ...
    with warnings.catch_warnings(
        record=True
    ):  # suppress annoying qmc.Sobol UserWarning
    ...
    print(
        f"First {num_init_evals} queries drawn at random. Best minimum thus far: {Y.min().item():.3f}"
    )

    while len(Y) < max_evals:
        print(f"=== Iteration {len(Y) + 1} ===", flush=True)
        ...
        print(
            f"Observed function value: {y_next:.3f}, Best function value seen thus far: {Y.min():.3f}"
        )
