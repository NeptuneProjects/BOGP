...
def main():
    ...
    print("=" * 50)
    print("Running GP/GIBBON")
    X_gibbon = gen.get_initial_points(TRUE_DIM, N_INIT, DTYPE, DEVICE)
    Y_gibbon = torch.tensor(objective(X_gibbon), dtype=DTYPE, device=DEVICE)
    # Y_ei = torch.tensor(
    #     [branin_emb(x) for x in X_ei], dtype=DTYPE, device=DEVICE
    # ).unsqueeze(-1)
    ...
    # Disable input scaling checks as we normalize to [-1, 1]
    with botorch.settings.validate_input_scaling(False):
        while len(Y_gibbon) < (BUDGET - len(Y_gibbon)):
            train_Y = (Y_gibbon - Y_gibbon.mean()) / Y_gibbon.std()
            likelihood = GaussianLikelihood(noise_constraint=Interval(1e-8, 1e-3))
            model = SingleTaskGP(X_gibbon, train_Y, likelihood=likelihood)
            mll = ExactMarginalLogLikelihood(model.likelihood, model)
            optimizer = torch.optim.AdamW([{"params": model.parameters()}], lr=0.1)
            model.train()
            model.likelihood.train()
            for _ in range(50):
                optimizer.zero_grad()
                output = model(X_gibbon)
                loss = -mll(output, train_Y.squeeze())
                loss.backward()
                optimizer.step()

            # Create a batch
            qGIBBON = qLowerBoundMaxValueEntropy(model, X_gibbon)
            candidate, acq_value = optimize_acqf(
                qGIBBON,
                bounds=torch.stack(
                    [
            ...
            # Append data
            X_gibbon = torch.cat((X_gibbon, candidate), axis=0)
            Y_gibbon = torch.cat((Y_gibbon, Y_next), axis=0)

            # Print current status
            print(f"Trial {len(X_gibbon)} | Best value: {1 - Y_gibbon.max().item():.3}")

    print(f"Complete; best parameters: {X_gibbon[Y_gibbon.argmax()]}")
